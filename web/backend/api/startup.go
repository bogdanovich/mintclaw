...
const (
	autoStartEntryName = "MintClawLauncher"
	launchAgentLabel   = "io.github.bogdanovich.mintclaw.launcher"
)
...
func linuxAutoStartPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "autostart", "mintclaw-web.desktop")
}
...
func setLinuxAutoStart(enabled bool, exePath string, args []string) error {
	...
	if enabled {
		...
		content := strings.Join([]string{
			...
			"Version=1.0",
			"Name=MintClaw Web",
			"Comment=Start MintClaw Web on login",
