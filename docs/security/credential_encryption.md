...
MintClaw supports encrypting `api_key`/`api_keys` values in `model_list` configuration entries.
...
```bash
export MINTCLAW_KEY_PASSPHRASE="your-passphrase"
```
...
| Encrypted | `enc://<base64>` | Decrypted at startup using `MINTCLAW_KEY_PASSPHRASE` |
...
aes_key = HKDF-SHA256(ikm, salt, info="mintclaw-credential-v1", 32 bytes)
```
...
1. The **passphrase** (`MINTCLAW_KEY_PASSPHRASE`)
...
|----------|----------|-------------|
| `MINTCLAW_KEY_PASSPHRASE` | Yes (for `enc://`) | Passphrase used for key derivation |
| `MINTCLAW_SSH_KEY_PATH` | No | Path to SSH private key. If not set, auto-detects from `~/.ssh/mintclaw_ed25519.key` |

### SSH Key Auto-Detection

If `MINTCLAW_SSH_KEY_PATH` is not set, MintClaw looks for the mintclaw-specific key:

```
~/.ssh/mintclaw_ed25519.key
```

This dedicated file avoids conflicts with the user's existing SSH keys.
Run `mintclaw onboard` to generate it automatically.

`os.UserHomeDir()` is used for cross-platform home directory resolution (reads `USERPROFILE` on Windows, `HOME` on Unix/macOS).

> **Note:** An SSH key file is required for credential encryption. If no key is found and `MINTCLAW_SSH_KEY_PATH` is not set, encryption/decryption will fail. Run `mintclaw onboard` to generate the key automatically.

---

## Migration

Because the only secret material is `MINTCLAW_KEY_PASSPHRASE` and the SSH private key file, migration is straightforward:

1. Copy the config file to the new machine.
2. Set `MINTCLAW_KEY_PASSPHRASE` to the same value.
3. Copy the SSH private key file to the same path (or set `MINTCLAW_SSH_KEY_PATH` to its new location).
...
- **Both passphrase and SSH key are required.** The SSH key acts as a second factor — without it, encryption/decryption will fail. Run `mintclaw onboard` to generate the key if it doesn't exist.
- **The SSH key is read-only at runtime.** MintClaw never writes to or modifies the SSH key file.
- **Plaintext keys remain supported.** Existing configs without `enc://` are unaffected.
- **The `enc://` format is versioned** via the HKDF `info` field (`mintclaw-credential-v1`), allowing future algorithm upgrades without breaking existing encrypted values.
