...
older companion catalog is quarantined during registry loading instead of
preventing the gateway from starting. The gateway removes the stale browser
command surface, changes the stored catalog hash, and marks an approved node
incompatible. No stale browser command can execute. A current companion must
reconnect with the new catalog, after which the owner explicitly renews the
command-surface approval before browser commands are available again.
