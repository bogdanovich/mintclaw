...
The durable registry and answer parser remain backward-compatible with records
containing multiple questions. New model-authored calls are deliberately
single-question so chat users can answer naturally without assembling a
multi-line `question_id: answer` payload.
