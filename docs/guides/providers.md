...
> [!NOTE]
> Voice transcription uses the model selected by `voice.model_name`.
...
By default, MintClaw now forwards tool JSON Schemas unchanged.
...
If `voice.model_name` is not configured, voice transcription is disabled.
...
- MintClaw still uses the OpenAI-compatible `/audio/speech` route for this setup.
...
MintClaw already supports automatic failover when you configure `primary` + `fallbacks` in the agent model settings.
...
In addition to per-request fallback, MintClaw now keeps a temporary session-scoped sticky failover state for errors that usually persist beyond one turn:
