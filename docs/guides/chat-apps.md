...
Talk to your mintclaw through Telegram, Discord, WhatsApp, Matrix, QQ, DingTalk, LINE, WeCom, Feishu, Slack, IRC, OneBot, MQTT, MaixCam, or MintClaw (native protocol)
...
| **MintClaw**             | ⭐ Easy            | Native MintClaw protocol channel                      |                                                                                                                  |
...
MintClaw now keeps command definitions in one shared registry. On startup, Telegram will automatically register supported bot commands (for example `/start`, `/help`, `/show`, `/list`, `/use`, `/btw`) so command menu and runtime behavior stay in sync.
...
If command registration fails (network/API transient errors), the channel still starts and MintClaw retries registration in the background.
...
MintClaw can connect to WhatsApp in two ways:
...
If `session_store_path` is empty, the session is stored in `<workspace>/whatsapp/`. Run `mintclaw gateway`; on first run, scan the QR code printed in the terminal with WhatsApp → Linked Devices.
...
MintClaw supports connecting to your personal WeChat account using the official Tencent iLink API.
...
4. Run `mintclaw gateway` and open QQ to chat with your bot
...
* Configure as shown above and run `mintclaw gateway`
...
MintClaw now exposes WeCom as a single AI Bot channel over WebSocket.
...
MintClaw connects to Feishu via WebSocket/SDK mode — no public webhook URL or callback server needed.
...
OneBot is an open protocol for QQ bots. MintClaw connects to any OneBot v11 compatible implementation (e.g., [Lagrange](https://github.com/LagrangeDev/Lagrange.Core), [NapCat](https://github.com/NapNeko/NapCatQQ)) via WebSocket.
...
Any MQTT client can communicate with MintClaw via a broker. Devices or services publish requests to the broker; MintClaw subscribes, processes them, and publishes responses back.
...
Username and password go in `~/.mintclaw/.security.yml`:
...
```
{prefix}/{agent_id}/{client_id}/request    # Client → MintClaw
{prefix}/{agent_id}/{client_id}/response   # MintClaw → Client
```
...
```bash
# Send a message
mosquitto_pub -t "/mintclaw/assistant/device1/request" \
  -m '{"text": "Hello"}'

# Subscribe to responses
mosquitto_sub -t "/mintclaw/assistant/device1/response"
```
