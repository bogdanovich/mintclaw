...
var (
	...
	ErrProviderAuthentication  = errors.New("browser provider authentication failed")
	ErrProviderProfileNotReady = errors.New("browser provider profile is not ready")
	ErrProviderQuota           = errors.New("browser provider quota is exhausted")
	ErrProviderTimeout         = errors.New("browser provider request timed out")
	ErrProviderUnavailable     = errors.New("browser provider is unavailable")
...
const (
	...
	ActionUpload      = browseraction.ActionUpload
...
func (prepared PreparedAction) Validate(maxTextBytes int) error {
	...
	case ActionDrag:
		if prepared.DestinationOrigin != "" || !elementRoleRegexp.MatchString(prepared.ElementRole) ||
			!elementRoleRegexp.MatchString(prepared.DestinationElementRole) || prepared.Effect != EffectUnknown ||
			prepared.InputDigest != "" || prepared.InputBytes != 0 {
			return fmt.Errorf("%w: malformed prepared drag", ErrInvalid)
		}
	case ActionPress:
