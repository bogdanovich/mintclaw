...
//   - Encrypted:   "enc://<base64>"     → AES-256-GCM decrypt via MINTCLAW_KEY_PASSPHRASE
...
//  2. MINTCLAW_SSH_KEY_PATH env var
//  3. ~/.ssh/mintclaw_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
const SSHKeyPathEnvVar = "MINTCLAW_SSH_KEY_PATH"

// mintclawHome is a package-local copy of config.EnvHome. It is kept here to
// avoid a circular import between pkg/credential and pkg/config.
const mintclawHome = "MINTCLAW_HOME"

const (
	FileScheme = "file://"
	EncScheme  = "enc://"

	hkdfInfo = "mintclaw-credential-v1"
	saltLen  = 16
	nonceLen = 12
	keyLen   = 32
)
...
//
// passphrase is required (MINTCLAW_KEY_PASSPHRASE value).
// sshKeyPath is the SSH private key file to use; pass "" to auto-detect via
// MINTCLAW_SSH_KEY_PATH env var or ~/.ssh/mintclaw_ed25519.key.
...
//   - exact match with MINTCLAW_SSH_KEY_PATH env var
//   - within the MINTCLAW_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with MINTCLAW_SSH_KEY_PATH.
	...
	// Within MINTCLAW_HOME.
	if mintclawHome := os.Getenv(mintclawHome); mintclawHome != "" {
		if isWithinDir(clean, mintclawHome) {
			return true
		}
	}
...
// Final key: HKDF-SHA256(ikm, salt, info="mintclaw-credential-v1", 32 bytes)
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set MINTCLAW_SSH_KEY_PATH or place key at ~/.ssh/mintclaw_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (MINTCLAW_SSH_KEY_PATH, MINTCLAW_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
...
//  2. MINTCLAW_SSH_KEY_PATH env var
//  3. ~/.ssh/mintclaw_ed25519.key (auto-detection)
//
...
// findDefaultSSHKey returns the mintclaw-specific SSH key path if it exists.
