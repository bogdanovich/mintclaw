...
MintClaw 不会把 ASR 的 API Key 放在 `voice` 配置里。
...
这种方式更明确、更安全，也和 MintClaw 其他模型配置方式保持一致。
...
- 你可以不写 `api_base`，MintClaw 会自动使用 Groq 默认接口地址。
