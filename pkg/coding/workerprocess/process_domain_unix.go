...
func (domain processDomain) stop(_ time.Duration) error {
	...
	if err := errors.Join(groupErr, leaderErr); err != nil {
		return fmt.Errorf("terminate coding worker process group: %w", err)
	}
	return nil
}

func (domain processDomain) close() error {
	// command.Wait has reaped the leader before close is called. Surviving
	// members retain the PGID, so this drains the owned Unix process group.
	// A deliberately detached setsid process is outside that portable boundary;
	// the parent separately bounds inherited diagnostic descriptors.
