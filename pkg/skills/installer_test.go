...
func TestParseGitHubRef(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantOwner:    "bogdanovich",
			wantRepoName: "mintclaw",
			wantRef:      "main",
			wantSubPath:  "",
		},
		{
			name:         "owner/repo with subpath",
			repo:         "bogdanovich/mintclaw/skills/test",
			wantOwner:    "bogdanovich",
			wantRepoName: "mintclaw",
			wantRef:      "main",
		...
		},
		{
			...
			repo:         "https://github.com/bogdanovich/mintclaw/tree/dev/skills/test",
			wantOwner:    "bogdanovich",
			wantRepoName: "mintclaw",
			wantRef:      "dev",
			wantSubPath:  "skills/test",
		},
		{
			...
			repo:         "https://github.com/bogdanovich/mintclaw/blob/main/README.md",
			wantOwner:    "bogdanovich",
			wantRepoName: "mintclaw",
			wantRef:      "main",
		...
		},
		{
			...
			repo:         "https://github.com/bogdanovich/mintclaw",
			wantOwner:    "bogdanovich",
			wantRepoName: "mintclaw",
			wantRef:      "main",
			wantSubPath:  "",
		...
		},
		{
			...
			repo:           "https://gitlab.com/bogdanovich/mintclaw/-/tree/main/skills/test",
			wantErr:        true,
...
func TestSkillInstaller_InstallFromGitHub_SkillAlreadyExists(t *testing.T) {
	...
	existingSkill := filepath.Join(skillsDir, "mintclaw")
	if err := os.MkdirAll(existingSkill, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(existingSkill, "SKILL.md"), []byte("existing"), 0o644); err != nil {
		t.Fatal(err)
	}

	// Try to install the same skill - should fail
	err = installer.InstallFromGitHub(context.Background(), "bogdanovich/mintclaw")
	if err == nil {
