...
func createClaudeAuthProvider() (LLMProvider, error) {
	...
	if cred == nil {
		return nil, fmt.Errorf("no credentials for anthropic. Run: mintclaw auth login --provider anthropic")
	}
...
func createCodexAuthProvider() (LLMProvider, error) {
	...
	if cred == nil {
		return nil, fmt.Errorf("no credentials for openai. Run: mintclaw auth login --provider openai")
	}
...
func CreateProviderFromConfig(cfg *config.ModelConfig) (LLMProvider, string, error) {
	...
	if userAgent == "" {
		userAgent = fmt.Sprintf("MintClaw/%s", config.Version)
	}
...
func CreateImageGenerationProviderFromModel(model string) (ImageGenerationProvider, string, error) {
	...
	case "", "openai", "openai-codex":
		...
		if !ok || !imageProvider.Capabilities().ImageGeneration.Supported {
