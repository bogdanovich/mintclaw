...
func TestMintClawChannel_HandleMessageSend_AllowsMediaOnly(t *testing.T) {
	mb := bus.NewMessageBus()
	bc := &config.Channel{Type: "mintclaw", Enabled: true, AllowFrom: []string{"mintclaw-user"}}
	ch, err := NewMintClawChannel(bc, &config.MintClawSettings{
		Token: *config.NewSecureString("test-token"),
	}, mb)
	if err != nil {
		t.Fatalf("NewMintClawChannel() error = %v", err)
	}
...
func newTestMintClawClientChannel(t *testing.T) (*MintClawClientChannel, *bus.MessageBus) {
	t.Helper()
...
func TestMintClawClientChannel_HandleServerMessage_ForwardsMedia(t *testing.T) {
	ch, mb := newTestMintClawClientChannel(t)
	pc := &mintclawConn{sessionID: "sess-media"}
	imageURL := "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+X2ioAAAAASUVORK5CYII="
...
func TestMintClawClientChannel_HandleInbound_ForwardsMediaCreate(t *testing.T) {
	ch, mb := newTestMintClawClientChannel(t)
	pc := &mintclawConn{sessionID: "sess-media-create"}
	imageURL := "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+X2ioAAAAASUVORK5CYII="

	ch.handleInbound(pc, MintClawMessage{
...
func TestMintClawClientChannel_HandleServerMessage_ForwardsTextWithDownloadAttachment(t *testing.T) {
	...
	ch.ctx = context.Background()
	pc := &mintclawConn{sessionID: "sess-download-attachment"}
...
func TestMintClawClientChannel_HandleServerMessage_ForwardsTextWithInvalidMediaPayload(t *testing.T) {
	...
	ch.ctx = context.Background()
	pc := &mintclawConn{sessionID: "sess-invalid-media"}
...
func TestMintClawClientChannel_HandleServerMessage_IgnoresThought(t *testing.T) {
	...
	ch.ctx = context.Background()
	pc := &mintclawConn{sessionID: "sess-thought"}
