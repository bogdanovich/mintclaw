...
func TestProcessHook_HelperProcess(t *testing.T) {
	if os.Getenv("MINTCLAW_HOOK_HELPER") != "1" {
		return
	}
...
func processHookHelperEnv(mode, eventLog string) []string {
	env := []string{
		"MINTCLAW_HOOK_HELPER=1",
		"MINTCLAW_HOOK_MODE=" + mode,
	}
	if eventLog != "" {
		env = append(env, "MINTCLAW_HOOK_EVENT_LOG="+eventLog)
	}
	return env
}
...
func runProcessHookHelper() error {
	mode := os.Getenv("MINTCLAW_HOOK_MODE")
	eventLog := os.Getenv("MINTCLAW_HOOK_EVENT_LOG")
