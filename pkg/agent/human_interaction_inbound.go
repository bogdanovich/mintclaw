...
func (al *AgentLoop) cancelInteractionForControlMessage(
	...
	command, ok := newCancelInteractionCommand(msg, target)
	if !ok {
		return interactionControlCancellationResult{}, nil
	}
	return newInteractionService(al).Cancel(ctx, command)
}
	...
	if approvalAllowed {
		...
		if _, resultIndex := interactionToolPairIndexes(
			...
			if resuming.ApprovalConsumedAt != 0 {
				...
				executor, err := al.prepareApprovedInteractionTool(ctx, registry, agent, resuming)
				if err != nil {
...
func (al *AgentLoop) prepareApprovedInteractionTool(
	ctx context.Context,
	registry *interactions.Registry,
	agent *AgentInstance,
	record interactions.Record,
	...
	executor.afterTool = func(audits []toolshared.WriteAuditEntry) error {
		if receipts := interactionOutcomeReceipts(audits); len(receipts) > 0 {
			...
			if _, err := registry.RecordOutcomeReceipts(current.ID, current.Revision, receipts); err != nil {
				return fmt.Errorf("persist approved action receipts: %w", err)
			}
		}
		return nil
	}
...
func (al *AgentLoop) deliverTaskInteractionFinal(
	...
	if task.DeliveryStatus == taskregistry.DeliveryPending {
		...
		al.tasks.updateDeliveryStatus(workspace, taskID, terminalStatus, completionID, "")
		task, ok = taskRegistry.Get(taskID)
		if !ok {
