...
steel-profile-reuse)
	...
	stage_one_checks='first_marker_absent, marker_seeded'
	stage_one_workflow="Open exactly one cloud session in this stage and call browser_observe exactly once to verify about:blank. Navigate to ${fixture_origin}/browser-smoke/check and call browser_observe exactly once on the untouched state. Record first_marker_absent from that observation: it is true only when local_storage=false. Click Set managed smoke marker exactly once with declared_effect=local_edit and call browser_observe exactly once; set marker_seeded from whether that observation shows local_storage=true. Close this session exactly once and require clean provider release. Do not call browser_session status, do not open the verification session in this stage, do not repeat any action or observation, and do not retry a failed predicate."
	stage_two=steel-profile-verify
	stage_two_checks='marker_reused, marker_cleared'
	stage_two_workflow="Open exactly one cloud session in this stage: it is the verification session that follows the seed stage. Call browser_observe exactly once to verify about:blank. Navigate to ${fixture_origin}/browser-smoke/check and call browser_observe exactly once; set marker_reused from whether that observation shows local_storage=true. Click Clear browser smoke state exactly once with declared_effect=local_edit and call browser_observe exactly once; set marker_cleared from whether that observation shows local_storage=false. Close this session exactly once with clean provider release. Do not call browser_session status, do not open a preparatory session, do not repeat any action or observation, and do not retry a failed predicate."
	;;
steel-handoff)
