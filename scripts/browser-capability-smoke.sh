...
steel-profile-reuse)
	...
	stage_two_checks='marker_reused, marker_cleared'
	stage_two_workflow="Open exactly one cloud session in this stage: it is the verification session that follows the seed stage. Do not open a preparatory session, do not repeat any part of this workflow, and do not retry a failed predicate. Call browser_observe to verify about:blank. Navigate to ${fixture_origin}/browser-smoke/check and call browser_observe until it shows local_storage=true. Click Clear browser smoke state with declared_effect=local_edit, call browser_observe until it shows local_storage=false, and close this session with clean provider release."
	;;
steel-handoff)
